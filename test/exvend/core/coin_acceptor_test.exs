defmodule CoinAcceptorTest do
  ...
  def all_possible_cashiers_change([], found_change, amount), do: found_change |> Enum.reject(&Enum.empty?/1)
  ...
  defp cashiers_change([h | _] = list, found_change, remaining_amount) when remaining_amount >= h do
